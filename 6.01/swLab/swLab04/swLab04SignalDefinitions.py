...
def polyR(s, n):  # if n is not the list , we should check polynomial
    signals = []
    maxR = len(n.coeffs)-1
    coeff = n.coeffs
    for idx, scale in enumerate(coeff):
        delay = maxR - idx
        signals.append(ScaledSignal(Rn(s, delay), scale))
    retSignal = signals[0]
    for other in signals[1:]:
        retSignal = SummedSignal(retSignal, other)
    return retSignal

## recursive
def polyRHandler(s, coeff, R):
    if coeff:
        return SummedSignal(ScaledSignal(Rn(s, R), coeff[0]), polyRHandler(s, coeff[1:], R-1))
    else:
...
def polyRRec(s, n):
    coeffs = n.coeffs
    R = len(coeffs) - 1
    return polyRHandler(s, coeffs, R)
