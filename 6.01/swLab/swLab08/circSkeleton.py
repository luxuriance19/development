...
################
    ...
    def addCurrent(self, current, node, sign):
        t_current = self.currents.get(node, [])
        t_current.append((current, sign))
        self.currents.update({node: t_current})
...
class OpAmp(Component):
    ...
    def getEquation(self):
        return le.Equation([self.K, -self.K, -1], [self.nPlus, self.nMinus, self.nOut], 0)
